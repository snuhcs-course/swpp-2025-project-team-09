...
class TestGetSessionInfoViewIntegration(TestCase):
    """Integration tests for Get Session Info endpoint"""
...
class TestSessionReviewViewIntegration(TestCase):
    """Integration tests for Session Review endpoint"""
...
class TestSessionReloadViewIntegration(TestCase):
    """Integration tests for Session Reload endpoint"""
    ...
    def setUp(self):
        ...
        # Create test user and session
        self.test_user = User.objects.create(
            device_info="test-reload-device",
            language_preference="en",
            created_at=timezone.now(),
        )
        self.test_session = Session.objects.create(
    ...
    def test_01_reload_session_no_pages(self):
        """Test reload session with no pages"""
        params = {
            "user_id": str(self.test_user.uid),
            "started_at": self.test_session.created_at.isoformat(),
            "page_index": 0,
        }
        response = self.client.get("/session/reload", params)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn("session_id", response.data)
        self.assertIsNone(response.data["image_base64"])

    def test_02_reload_session_with_page(self):
        """Test reload session with page data"""
        # Create test image
